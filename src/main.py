...
async def generate(channel, convo):
    ...
    try:
        completion = await aio.get_event_loop().run_in_executor(None, lambda: ai.ChatCompletion.create(model=gpt_model, messages=convo, max_tokens=max_tokens))
        completion = completion.choices[0].message["content"]
    finally:
        stop_event.set()
        await typing_task
        typing_task.cancel()
    for replacement in replacements: completion = completion.replace(replacement[0], replacement[1])
    return completion

class UserData:

    def __init__(self, member: discord.Member, path=os.path.join(os.path.dirname(__file__), file_paths['user_data'])): self.path, self.member, self.user_id = path, member, member.id

    async def load_data(self): return json.load(open(self.path, 'r'))

    async def save_data(self, data): json.dump(data, open(self.path, 'w'), indent=4)

    async def create_user(self):
        data = await self.load_data()
        default_user = next((user for user in data["users"] if user["id"] == 0), None)
        if default_user is None: raise ValueError("Default user not found")
        if any(user["id"] == self.user_id for user in data["users"]): return None
    ...
    async def set_user_value(self, key, value=None):
        ...
        if key not in user and key in default_user: user[key] = default_user[key]
        if value is not None: user[key] = value
        await self.save_data(data)
    ...
    async def delete_user_value(self, key):
        ...
        if key not in user: raise ValueError("Key not found in user data")
        del user[key]
        await self.save_data(data)
...
async def execute_encoded_message(message, member: discord.Member, convo):
    ...
    parameters = split_executor[1:] if len(split_executor) > 1 else None
...
def save_to_transcript(first_message: discord.Message, convo):
    ...
    else: transcript.append({"transcript_id": transcript_id,"author_id": first_message.author.id,"date": datetime.now().strftime("%Y-%m-%d"),"time": datetime.now().strftime("%H:%M:%S"),"tokens": total_tokens,"conversation": {"message_count": len(convo),"messages": convo}})
    json.dump(transcript, open(transcript_path, 'w'), indent=4)
    ...
    if any(trigger_message_content.startswith(trigger) for trigger in triggers) and not author.bot:
        if (await UserData(author).get_user_value('interaction_method') == "THREAD") and not isinstance(ctx.channel, discord.channel.DMChannel):
            ...
            title = config('message_formats')['thread_title'].format(
                # You may add your own formatting here
                user = author.name,
            ...
            save_to_transcript(ctx, convo)
            response, convo = await execute_encoded_message(response, author, convo)
            for m in (response[i:i+2000] for i in range(0, len(response), 2000)): await ctx.reply(m,mention_author=False)
            if convo[-1]["role"] == "system":
                response = await generate(ctx.channel, convo)
                convo.append({"content": response, "role": "assistant"})
                save_to_transcript(ctx, convo)
                for m in (response[i:i+2000] for i in range(0, len(response), 2000)): await ctx.reply(m,mention_author=False)
            conversationHappening = True
            def check(m): return m.content is not None and m.author.id == authorID
            while conversationHappening:
                try: userMessage = await client.wait_for("message", timeout=1200, check=check)
                except:
                    conversationHappening = False
